(function ( $ ) {
	...
	$.fn.skillset = function( options ) {
		...
		//modificado por Carlos para que el tamaño de la pantalla no afecte
		//if($(this).isOnScreen() && !$(this).checkRun() ){
			create_trigger($(this));

		//}

		function bar_loop(start,value,length,average){
			...
			for(var i = 0; i < length; i++){
				...
				setTimeout(function(){
					if(j == average){
						var html = '<div class="average"></div>';
		...
		function create_trigger(element){
			...
			for(key in settings.object) {
				//modificado por Carlos para no necesitar una variable fuera "object"
			...
			for(var i = 0; i < count; i++){
				...
				var average = settings.object[i]['average'];
				bar_loop(html,value,length,average);
